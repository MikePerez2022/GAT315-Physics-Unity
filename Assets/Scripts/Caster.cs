    ...
    void Update()
    {
        switch (type)
        {
            case eType.Ray:
                ...
                break;
            case eType.Sphere:
                Physics.SphereCastAll(transform.position, size, Vector3.forward, layerMask);
                raycastHits = Physics.SphereCastAll(transform.position, size * 0.5f, Vector3.forward, distance, layerMask);
                break;
            case eType.Box:
                raycastHits = Physics.BoxCastAll(transform.position, Vector3.one * size * 0.5f, Vector3.forward, transform.rotation, distance, layerMask);
                break;
        }
    }
