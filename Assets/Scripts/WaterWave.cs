...
public class WaterWave : MonoBehaviour
{
	...
	[SerializeField][Range(1, 120)] float xSize = 120;
	[SerializeField][Range(1, 120)] float zSize = 120;
	[SerializeField][Range(2, 120)] int xVertexNum = 120;
	[SerializeField][Range(2, 120)] int zVertexNum = 120;
	...
	Vector3 GerstnerWave(Vector3 position, Vector2 direction, float speed, float length, float amplitude, float roll)
    {
		...
		if(f == float.NegativeInfinity ||  f == float.PositiveInfinity) return v;
